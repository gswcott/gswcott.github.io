...
export class Gameover {
    ...
    init(){
        ...
        generate = true;
        playButton = new Button(this.globals.assets.imgFont, "RETRY", 0, this.vScreen.vH / 2, this.vScreen.vW, 100, this.config.scaleTitle, [255, 153, 0, 0.7]);
        bubbles = []; 
    ...
    play(){
        if(isInButton(this.vScreen.mouseX, this.vScreen.mouseY, playButton)){
            ...
            const fade = new FadeEnd(this.globals, "game", 0);
            enter(fade); 
        }
    }
    update(dt){
    ...
    draw(){
        ...
        this.config.scaleTitle = drawTitle(this.ctx, "GAME", this.globals.assets.imgFont, 
        0, this.vScreen.vH * 0.5 / 6, this.vScreen.vW, this.vScreen.vH * 1 / 6, this.config.scaleTitle); 
        drawTitle(this.ctx, "OVER", this.globals.assets.imgFont, 
        0, this.vScreen.vH * 1 / 6, this.vScreen.vW, this.vScreen.vH * 1.5 / 6, this.config.scaleTitle); 
        drawTitle(this.ctx, "SCORE:" + this.score, this.globals.assets.imgFont, 
        0, this.vScreen.vH * 1.5 / 6, this.vScreen.vW, this.vScreen.vH / 2, this.config.scaleTitle * 0.8); 
